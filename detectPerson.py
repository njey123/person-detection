...
def detect_person(image):
    ...
    image = imutils.resize(image, height=(min(300, image.shape[1])))
    ...
    # Show the output image (with the detected person)
    cv2.imshow("Detected Person - After NMS", image)

    try:
        return direction, isPersonStopping
    except NameError:
        return 'Person not found', True
